...
|---|---|---|
| **OAuth flow** | App installs cleanly on a fresh dev store; session token issued and refreshed without manual code. | Handled entirely by `@shopify/shopify-app-react-router`'s `authenticate.admin(request)` — no custom OAuth logic exists or is needed. Verify by uninstalling and reinstalling the app on the dev store once before beta. |
